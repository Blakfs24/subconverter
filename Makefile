...
$(DEPS_STAMP_DIR)/yaml-cpp: $(YAML_CPP_SRC_DIR)/.git | $(DEPS_STAMP_DIR)
	...
	cd $(DEPS_BUILD_DIR)/yaml-cpp && $(CMAKE) -DCMAKE_BUILD_TYPE=Release -DYAML_CPP_BUILD_TESTS=OFF -DYAML_CPP_BUILD_TOOLS=OFF -DBUILD_SHARED_LIBS=OFF -DCMAKE_POSITION_INDEPENDENT_CODE=ON -DCMAKE_INSTALL_PREFIX=$(ABS_DEPS_INSTALL_DIR) -DCMAKE_POLICY_VERSION_MINIMUM=3.5 $(ABS_YAML_CPP_SRC_DIR)
...
$(DEPS_STAMP_DIR)/pcre2: $(PCRE2_SRC_DIR)/.git | $(DEPS_STAMP_DIR)
	...
	cd $(DEPS_BUILD_DIR)/pcre2 && $(CMAKE) -DCMAKE_BUILD_TYPE=Release -DBUILD_SHARED_LIBS=OFF -DPCRE2_BUILD_TESTS=OFF -DPCRE2_BUILD_PCRE2_16=OFF -DPCRE2_BUILD_PCRE2_32=OFF -DPCRE2_BUILD_PCRE2GREP=OFF -DPCRE2_SUPPORT_JIT=ON -DCMAKE_INSTALL_PREFIX=$(ABS_DEPS_INSTALL_DIR) -DCMAKE_POLICY_VERSION_MINIMUM=3.5 $(ABS_PCRE2_SRC_DIR)
